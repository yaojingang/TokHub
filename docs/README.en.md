...
TokHub 2.0 gives regular users their own AI service connection entry. After signing in, a user can connect an official API key or use a deployment-enabled ChatGPT, Gemini, or DeepSeek authorization flow. A verified connection can create a personal OpenAI-compatible relay and a dedicated Gateway Key.
...
| Upstream credentials | API keys configured by a platform or workspace | Adds user-owned API keys, official OAuth, and controlled experimental authorization |
...
flowchart LR
    ...
    C1 --> D["Real minimal generation check"]
    C2 --> D
    C3 --> D
    C4 --> D
...
- TokHub stores a connection only after a real minimal generation check. The server encrypts credentials with a versioned AES-256-GCM keyring.

## Provider Support Matrix

| Provider | Official API key | Account authorization | Current level | Notes |
| --- | --- | --- | --- | --- |
| ChatGPT / OpenAI | Supported | Codex OAuth | Experimental | Personal scope, one relay, and a server-enforced low QPS limit |
| Gemini | Supported | Official Google OAuth | Available after configuration | Requires an OAuth client, Cloud Project, HTTPS callback, and Redis |
| Kimi | Supported | Unavailable | Stable | Supports mainland China and international endpoints |
| DeepSeek | Supported | Open Platform guide and web account session | Stable API key, experimental web flow | The web path uses an isolated DS2API bridge for validation and forwarding |
...
3. Choose an official API key, official OAuth, or a deployment-enabled experimental method.
4. Complete the provider sign-in or paste a developer credential.
5. TokHub verifies account identity, model access, and a minimal generation request.
...
- API key, OAuth, and experimental connections all require a real minimal generation check.
