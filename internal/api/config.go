...
type Config struct {
	...
	AILabModeEnabled                  bool
	AIOfficialClientEnabled           bool
	AIOfficialClientTaskTimeout       time.Duration
	AIOfficialClientTrustedProxyCIDRs []string
	AIProviderKillSwitches            map[string]bool
	AIProviderTermsDigests            map[string]string
...
func LoadConfig() Config {
	...
	labModeEnabled := envBool("TOKHUB_AI_LAB_MODE", false)
	opencliBrowserEnabled := labModeEnabled && envBool("TOKHUB_AI_OPENCLI_BROWSER_EXPERIMENTAL", false) &&
	...
	return Config{
		...
		AILabModeEnabled:                  labModeEnabled,
		AIOfficialClientEnabled:           envBool("TOKHUB_AI_OFFICIAL_CLIENT_ENABLED", false),
		AIOfficialClientTaskTimeout:       envDuration("TOKHUB_AI_OFFICIAL_CLIENT_TASK_TIMEOUT", 3*time.Minute, 30*time.Second, 10*time.Minute),
		AIOfficialClientTrustedProxyCIDRs: parseCSV(os.Getenv("TOKHUB_AI_OFFICIAL_CLIENT_TRUSTED_PROXY_CIDRS")),
		AIProviderKillSwitches: map[string]bool{
			"openai":   envBool("TOKHUB_AI_OPENAI_KILL_SWITCH", false),
			"gemini":   envBool("TOKHUB_AI_GEMINI_KILL_SWITCH", false),
			"deepseek": envBool("TOKHUB_AI_DEEPSEEK_KILL_SWITCH", false),
			"grok":     envBool("TOKHUB_AI_GROK_KILL_SWITCH", false),
		},
		AIProviderTermsDigests: map[string]string{
			"openai":   strings.TrimSpace(os.Getenv("TOKHUB_AI_OPENAI_TERMS_DIGEST")),
			"gemini":   strings.TrimSpace(os.Getenv("TOKHUB_AI_GEMINI_TERMS_DIGEST")),
			"deepseek": strings.TrimSpace(os.Getenv("TOKHUB_AI_DEEPSEEK_TERMS_DIGEST")),
			"grok":     strings.TrimSpace(os.Getenv("TOKHUB_AI_GROK_TERMS_DIGEST")),
		},
		AIGeminiOAuthEnabled:         envBool("TOKHUB_AI_GEMINI_OAUTH_ENABLED", false),
		AIChatGPTCodexExperimental:   labModeEnabled && envBool("TOKHUB_AI_CHATGPT_CODEX_EXPERIMENTAL", false),
		AIDeepSeekGuidedEnabled:      envBool("TOKHUB_AI_DEEPSEEK_GUIDED_ENABLED", true),
		AIDeepSeekWebExperimental:    labModeEnabled && envBool("TOKHUB_AI_DEEPSEEK_WEB_EXPERIMENTAL", false),
